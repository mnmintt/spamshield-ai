...
An intelligent spam detection web application built with NLP and Machine Learning. Detects spam in typed messages, uploaded files (PDF, DOCX, TXT), images, and bulk CSV uploads using both traditional ML models and advanced transformer-based NLP (DistilBERT). Includes LIME-powered explainable AI (XAI) to show why the model made each decision.
...
| Chan Zhi Chao | Model Training & Evaluation |
...
| XAI Explanations | LIME-powered word contribution charts showing exactly which words pushed the model towards spam or safe — available in Text Analyzer, File Scanner, Image Scanner, and Batch Prediction (SVM model) |
...
| Explainable AI (XAI) | LIME (lime, lime.lime_text) |
...
This project uses **LIME (Local Interpretable Model-agnostic Explanations)** to make the SVM model's decisions transparent. After every prediction, a word contribution chart is generated showing:

- **Red bars** — words that pushed the model towards classifying the message as **Spam**
- **Green bars** — words that pushed the model towards classifying the message as **Safe**
- Bar length indicates the strength of each word's influence

XAI explanations are available across all four input modes: Text Analyzer, File Scanner, Image Scanner, and Batch Prediction (representative samples). LIME is only applicable to the SVM + TF-IDF model; DistilBERT does not expose XAI explanations.
